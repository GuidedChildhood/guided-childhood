...
export default function Error({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div style={{ minHeight: '72dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '48px 24px' }}>
      <p style={{ fontFamily: 'var(--font-mono)', fontSize: '14px', fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--terracotta)', marginBottom: '12px' }}>
      ...
      <p style={{ fontSize: '18px', lineHeight: 1.6, color: 'var(--ink-soft)', maxWidth: '460px', marginBottom: '28px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center' }}>
        <button onClick={reset} style={{ background: 'var(--terracotta)', color: 'var(--ink)', border: 'none', cursor: 'pointer', borderRadius: '16px', padding: '13px 22px', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '17px', boxShadow: '0 5px 0 var(--terracotta-dark)' }}>
          Try again
        </button>
        <Link href="/dashboard" style={{ background: '#fff', color: 'var(--ink)', textDecoration: 'none', border: '1.5px solid var(--border)', borderRadius: '16px', padding: '13px 22px', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '17px' }}>
