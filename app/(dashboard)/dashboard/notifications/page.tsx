  ...
  return (
    <div style={{ maxWidth: '620px', margin: '0 auto', padding: '26px 20px 48px' }}>
      <div style={{ marginBottom: '22px' }}>
        ...
        <p style={{ color: 'var(--ink-soft)', fontSize: 'var(--text-lg)', lineHeight: 1.6 }}>
          {items.length === 0
      ...
      {items.length === 0 ? (
        <div style={{ background: 'var(--cream)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-card)', padding: '40px 24px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 'var(--text-md)', color: 'var(--ink-muted)', lineHeight: 1.6, margin: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '13px' }}>
          {bothGroups && (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#B93B3F', margin: '2px 2px 0' }}>
              Waiting on you
            </p>
          )}
          {urgent.map(n => <NotificationCard key={n.id} n={n} />)}
          {bothGroups && (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-muted)', margin: '10px 2px 0' }}>
              Also here
            </p>
          )}
