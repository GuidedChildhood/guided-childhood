  ...
  return (
    <div style={{ maxWidth: '620px', margin: '0 auto', padding: '26px 20px 48px' }}>
      <div style={{ marginBottom: '22px' }}>
        ...
        <p style={{ color: 'var(--ink-soft)', fontSize: '17.5px', lineHeight: 1.6 }}>
          {items.length === 0
      ...
      {items.length === 0 ? (
        <div style={{ background: 'var(--cream)', border: '1px solid var(--border)', borderRadius: '20px', padding: '40px 24px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '16.5px', color: 'var(--ink-muted)', lineHeight: 1.6, margin: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '13px' }}>
          {bothGroups && (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '13px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#B93B3F', margin: '2px 2px 0' }}>
              Waiting on you
            </p>
          )}
          {urgent.map(n => <NotificationCard key={n.id} n={n} />)}
          {bothGroups && (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '13px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-muted)', margin: '10px 2px 0' }}>
              Also here
            </p>
          )}
