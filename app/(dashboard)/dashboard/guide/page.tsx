  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: '24px 20px 48px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '36px' }}>
        {HOW_IT_WORKS.map((step, i) => (
          <div key={step.title} style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: '18px', padding: '18px 20px', boxShadow: '0 5px 20px rgba(46,40,24,0.06)' }}>
            ...
            <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.55, margin: '0 0 12px' }}>{step.body}</p>
            <Link href={step.href} style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)', color: 'var(--terracotta-dark)', textDecoration: 'none' }}>
              {step.cta} →
            </Link>
          </div>
        ))}
      ...
      <div style={{ background: 'var(--deep-teal)', borderRadius: '20px', padding: '22px', marginBottom: '20px' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', letterSpacing: '-0.02em', color: '#fff', margin: '0 0 6px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
              {band.lessons.map(l => (
                <div key={l} style={{ display: 'flex', alignItems: 'flex-start', gap: '9px' }}>
                  ...
                  <span style={{ fontSize: 'var(--text-base)', color: 'var(--ink)', lineHeight: 1.45 }}>{l}</span>
                </div>
              ))}
            </div>
          </div>
