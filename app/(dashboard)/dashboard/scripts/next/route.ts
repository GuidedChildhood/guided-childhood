...
export async function GET(req: NextRequest) {
  ...
  if (!stage) return NextResponse.redirect(`${origin}/dashboard/scripts?from=pathway`)

  const list = `${origin}/dashboard/scripts?stage=${stage}&from=pathway`
  ...
  return NextResponse.redirect(`${origin}/dashboard/scripts/${pick.sort_order}?from=pathway&stage=${stage}`)
}
