...
export default async function PhoneSetupPage() {
  ...
  return (
    <div style={{ maxWidth: '680px', margin: '0 auto', padding: '24px 20px 48px' }}>
      <Link href="/dashboard" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: 'var(--text-sm)', color: 'var(--ink-muted)', textDecoration: 'none', fontFamily: 'var(--font-mono)', letterSpacing: '0.04em', marginBottom: '18px' }}>
        ← Home
      </Link>
      ...
      <p style={{ color: 'var(--ink-soft)', fontSize: 'var(--text-md)', lineHeight: 1.6, marginBottom: '18px' }}>
      ...
      <div style={{ background: 'var(--tint-sage)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-btn)', padding: '16px 18px', marginBottom: '26px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '30px' }}>
        {PHONE_LADDER.map((rung, i) => (
          <div key={i} style={{ display: 'flex', gap: '13px', background: '#fff', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-tile)', padding: '13px 15px' }}>
            <span aria-hidden style={{ flexShrink: 0, fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--terracotta-dark)', background: 'var(--terracotta-lt)', borderRadius: 'var(--radius-pill)', padding: '3px 9px', height: 'fit-content' }}>{rung.ages}</span>
            <div style={{ minWidth: 0 }}>
              ...
              <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, margin: 0 }}>{rung.detail}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {apps.length > 0 && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '12px', marginBottom: '10px' }}>
            {apps.map(a => (
              <a key={a.app_key} href={a.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', background: '#fff', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-btn)', padding: '14px 15px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <span aria-hidden style={{ width: 40, height: 40, borderRadius: 11, background: 'var(--cream)', border: 'var(--edge)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--text-xl)', lineHeight: 1, boxSizing: 'border-box' }}>{a.emoji}</span>
      ...
      <Link href="/dashboard/lessons" style={{ textDecoration: 'none', display: 'block', marginTop: '20px' }}>
        <div style={{ background: 'var(--stage-2)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-btn)', padding: '18px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px' }}>
          <div>
            ...
            <div style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, marginTop: '4px' }}>
...
function PhoneCard({ g }: { g: PhoneGuide }) {
  return (
    <div style={{ background: '#fff', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-card)', padding: '16px 18px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
        <span aria-hidden style={{ flexShrink: 0, width: 42, height: 42, borderRadius: 'var(--radius-tile)', background: 'var(--cream)', border: 'var(--edge)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--text-xl)' }}>{g.emoji}</span>
      ...
      <details style={{ marginTop: '12px' }}>
        <summary style={{ cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)', color: 'var(--terracotta-dark)', listStyle: 'none' }}>
        ...
        <div style={{ marginTop: '12px' }}>
          ...
          <div style={{ background: 'var(--cream)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '11px 13px', marginTop: '14px' }}>
            ...
            <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.55, margin: 0 }}>{g.time_management}</p>
          </div>

          <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-muted)', lineHeight: 1.5, margin: '12px 0 0' }}>{g.parental_tool}</p>
          <a href={g.official_url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', marginTop: '10px', fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--terracotta-dark)', textDecoration: 'none' }}>
...
function StepList({ label, steps, tint }: { label: string; steps: { name: string; how: string }[]; tint: string }) {
  return (
    <div>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-muted)', marginBottom: '8px' }}>{label}</div>
      <ol style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '9px' }}>
        {steps.map((s, i) => (
          <li key={i} style={{ display: 'flex', gap: '9px' }}>
            <span aria-hidden style={{ flexShrink: 0, width: 18, height: 18, borderRadius: '50%', background: tint, color: 'var(--ink)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, marginTop: 1 }}>{i + 1}</span>
...
function SectionLabel({ eyebrow, title, note }: { eyebrow: string; title: string; note?: string }) {
  return (
    <div style={{ margin: '0 0 13px' }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--terracotta-dark)', marginBottom: '3px' }}>{eyebrow}</div>
      <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-lg)', color: 'var(--ink)' }}>{title}</div>
      {note && <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-muted)', lineHeight: 1.5, margin: '3px 0 0' }}>{note}</p>}
    </div>
  )
}
