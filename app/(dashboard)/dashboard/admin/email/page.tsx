...
export default async function EmailHealthPage() {
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 20px 60px' }}>
      ...
      <p style={{ fontSize: 'var(--text-md)', color: 'var(--ink-soft)', lineHeight: 1.55, margin: '0 0 22px' }}>
      ...
      {h && (
        <>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 'var(--text-md)', color: 'var(--ink)', lineHeight: 1.55, margin: 0 }}>{h.detail}</p>
          </div>
          ...
          <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 20, padding: '18px 20px', marginBottom: 18 }}>
            ...
            {h.config.map(c => (
              <div key={c.key} style={{ display: 'flex', gap: 11, alignItems: 'flex-start', marginBottom: 14 }}>
                <span aria-hidden style={{ flexShrink: 0, fontSize: 'var(--text-lg)', lineHeight: 1.3 }}>
                  {c.set ? '✅' : c.fatal ? '🛑' : '⚠️'}
                </span>
                <span style={{ minWidth: 0 }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--ink)' }}>{c.key}</span>
                  <span style={{ display: 'block', fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.45, marginTop: 2 }}>
                    {c.why}
                  </span>
                  {c.value && (
                    <span style={{ display: 'block', fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', color: 'var(--ink-muted)', marginTop: 3, wordBreak: 'break-all' }}>
                      {c.value}
                    </span>
                  )}
                </span>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {h.byKey.map(k => (
                  <li key={k.key} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '8px 0', borderBottom: '1px dotted var(--border)' }}>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', color: 'var(--ink)', wordBreak: 'break-all' }}>{k.key}</span>
                    <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink)', flexShrink: 0 }}>{k.count}</span>
                  </li>
                ))}
              </ul>
            )}
