          ...
          <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 20, padding: '18px 20px', marginBottom: 18 }}>
            ...
            {h.services.map(s => (
              <div key={s.key} style={{ display: 'flex', gap: 11, alignItems: 'flex-start', marginBottom: 14 }}>
                <span aria-hidden style={{ flexShrink: 0, fontSize: 'var(--text-md)', lineHeight: 1.4 }}>{TONE[s.level].dot}</span>
                <span style={{ minWidth: 0 }}>
                  <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink)' }}>
                    {s.label}
                  </span>
                  <span style={{ fontSize: 'var(--text-base)', color: TONE[s.level].text, fontWeight: 700, marginLeft: 8 }}>{s.headline}</span>
              ...
              {[...h.jobs]
                ...
                .map(j => (
                  <li key={j.job.path} style={{ display: 'flex', gap: 11, alignItems: 'flex-start', padding: '10px 0', borderBottom: '1px dotted var(--border)' }}>
                    <span aria-hidden style={{ flexShrink: 0, fontSize: 'var(--text-base)', lineHeight: 1.5 }}>{TONE[j.level].dot}</span>
                    <span style={{ minWidth: 0, flex: 1 }}>
                      ...
                      <span style={{ display: 'block', fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)', marginTop: 3, wordBreak: 'break-all' }}>
