...
export default function TrackerForm({
  ...
  if (submitted) {
    return (
      <div style={{ maxWidth: '560px', margin: '0 auto', padding: '40px 20px', textAlign: 'center' }}>
        <div style={{ fontSize: 'var(--text-3xl)', marginBottom: '16px' }}>✓</div>
        <h2 style={{ fontSize: 'var(--text-xl)', marginBottom: '12px' }}>Check in saved</h2>
        <p style={{ color: 'var(--ink)', marginBottom: '24px', fontSize: 'var(--text-md)' }}>
          {avgScore && avgScore >= 4
        ...
        <div style={{ display: 'block' }}>
          <Link href="/dashboard" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', color: 'var(--ink)', textDecoration: 'none' }}>
            Back to dashboard
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '24px 20px' }}>
      ...
      {streakWeeks > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--cream)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-tile)', padding: '14px 18px', marginBottom: '18px' }}>
      ...
      {history.length > 0 && (
        <div style={{ marginBottom: '24px' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-muted)', marginBottom: '10px' }}>
            Your recent weeks
          </div>
      ...
      {currentWeekCheck && (
        <div style={{ background: 'var(--stage-2)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '12px 16px', marginBottom: '20px', fontSize: 'var(--text-base)', color: 'var(--ink)' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
        {QUESTIONS.map(q => (
          <div key={q.id}>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 'var(--text-lg)', color: 'var(--ink)', marginBottom: '14px', lineHeight: 1.4 }}>
              {q.label}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '6px' }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)' }}>{q.low}</span>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)' }}>{q.high}</span>
            </div>
          </div>
        ))}
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="input"
            style={{ resize: 'vertical', fontFamily: 'var(--font-body)', fontSize: 'var(--text-md)' }}
          />
        </div>
