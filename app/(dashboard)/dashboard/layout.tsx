...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [ticksRes, asksRes, profileRes, kidsRes, sheetsRes, wordRes] = user
    ? await Promise.all([
        ...
        supabase.from('printable_completions').select('id', { count: 'exact', head: true })
          .eq('user_id', user.id).eq('status', 'pending'),
        // DiGi's word, unread. A butter badge on the DiGi tab, never red:
        // red is for a person waiting, and DiGi can wait.
        supabase.from('digi_prompts').select('id', { count: 'exact', head: true })
          .eq('user_id', user.id).eq('kind', 'insight').eq('status', 'pending'),
      ])
    : [{ count: 0 }, { count: 0 }, { data: null }, { data: [] }, { count: 0 }, { count: 0 }]
  const digiWord = (wordRes as { count: number | null }).count ?? 0
    ...
    <div className="gc-shell gc-dash" style={{ minHeight: '100dvh', background: 'var(--app-bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{
        ...
        <div style={{ maxWidth: '1080px', margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', height: '64px', gap: '32px' }}>
          ...
          <Suspense fallback={null}><NavTabs pendingAsks={pendingAsks ?? 0} digiWord={digiWord} /></Suspense>
      ...
      <Suspense fallback={null}><MobileTabBar pendingAsks={pendingAsks ?? 0} digiWord={digiWord} /></Suspense>
