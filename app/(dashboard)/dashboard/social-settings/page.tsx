...
export default async function SocialSettingsPage() {
  ...
  return (
    <div style={{ maxWidth: '680px', margin: '0 auto', padding: '24px 20px 48px' }}>
      ...
      <p style={{ color: 'var(--ink-soft)', fontSize: '17px', lineHeight: 1.6, marginBottom: '16px' }}>
      ...
      <div style={{ background: 'var(--stage-1)', border: '1.5px solid var(--stage-1)', borderRadius: '14px', padding: '13px 16px', marginBottom: '26px' }}>
        <p style={{ fontSize: '15.5px', color: 'var(--ink)', lineHeight: 1.55, margin: 0 }}>
      ...
      <p style={{ fontSize: '14px', color: 'var(--ink-muted)', lineHeight: 1.6, margin: '18px 0 0', textAlign: 'center' }}>
    ...
    <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: '18px', padding: '16px 18px', opacity: dim ? 0.82 : 1 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', flexWrap: 'wrap' }}>
            ...
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.04em', color: 'var(--ink-muted)' }}>
              Account age {g.min_age}, not a readiness signal
            </span>
          </div>
          <p style={{ fontSize: '15px', color: 'var(--ink-soft)', lineHeight: 1.55, margin: '5px 0 0' }}>{g.blurb}</p>
        </div>
      </div>
      ...
      <details style={{ marginTop: '12px' }}>
        ...
        <div style={{ marginTop: '12px' }}>
          ...
          <ol style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '9px' }}>
            {g.settings.map((s, i) => (
              <li key={i} style={{ display: 'flex', gap: '9px' }}>
                <span aria-hidden style={{ flexShrink: 0, width: 18, height: 18, borderRadius: '50%', background: 'var(--tint-sage)', color: 'var(--ink)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, marginTop: 1 }}>{i + 1}</span>
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11.5px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-muted)', margin: '14px 0 7px' }}>
            Watch for
          </div>
          <ul style={{ margin: 0, paddingLeft: '18px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
            {g.watch_fors.map((w, i) => (
              <li key={i} style={{ fontSize: '15px', color: 'var(--ink-soft)', lineHeight: 1.5 }}>{w}</li>
            ))}
          </ul>
          ...
          <div style={{ background: 'var(--cream)', border: '1px solid var(--border)', borderRadius: '12px', padding: '11px 13px', marginTop: '14px' }}>
            ...
            <p style={{ fontSize: '14.5px', color: 'var(--ink-soft)', lineHeight: 1.5, margin: 0 }}>{g.supervision}</p>
          </div>

          <a href={g.official_url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', marginTop: '12px', fontSize: '15px', fontWeight: 700, color: 'var(--terracotta-dark)', textDecoration: 'none' }}>
