...
import JobBoardRow from '@/components/quests/JobBoardRow'
        ...
        <section style={CARD}>
          ...
          {mine.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {/* 10 rather than the 7 these rows used to sit at. Each card now
                  casts a hard 4px ink shadow, and at 7 the shadow of one lands
                  on the edge of the next, which reads as a printing error. */}
              {mine.map((q, i) => (
                <JobBoardRow
                  key={q.id}
                  job={q}
                  ...
                  busy={busy}
                  pinged={pinged}
                  stepsOpen={stepsOpen === q.id}
                  stepsDraft={stepsDraft}
                  setStepsOpen={open => setStepsOpen(open ? q.id : null)}
                  setStepsDraft={setStepsDraft}
                  onToggleFamily={() => updateJob(q.id, { is_family_job: !q.is_family_job })}
                  onSaveSteps={async steps => { await updateJob(q.id, { steps }); setStepsOpen(null) }}
                  onRemind={() => remind(q.title)}
