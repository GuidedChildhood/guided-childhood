...
export default async function KidPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  let assignedPrintable: { key: string; title: string; emoji: string; stars: number; sheetUrl: string; pdfColourIn?: string; previewUrl: string; sheetHeading?: { name: string; kicker: string }; extraSheetUrls?: string[] } | null = null
  {
    ...
    if (p) assignedPrintable = { key: p.key, title: p.title, emoji: p.emoji, stars: p.stars, sheetUrl: p.sheetUrl, pdfColourIn: p.pdfColourIn, previewUrl: p.previewUrl, sheetHeading: p.sheetHeading, extraSheetUrls: p.extraSheetUrls }
  }
