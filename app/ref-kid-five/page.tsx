...
function Fixture() {
  ...
  const view = params.get('view') ?? 'part'
  const theme = resolveTheme(params.get('accent'))
  const [open, setOpen] = useState(view === 'streak')
  // Something kind leading, for the sheet. pickDay is seeded, so asking for it
  // by name beats hunting for a date whose draw happens to include it.
    ...
    <div style={{ minHeight: '100dvh', background: theme.bg, padding: '22px 16px 50px', fontFamily: 'var(--font-body)' }}>
      <div style={{ maxWidth: '560px', margin: '0 auto' }}>
        <p style={{
          ...
          letterSpacing: '0.2em', textTransform: 'uppercase', color: theme.inkMuted, margin: '0 0 16px',
        }}>
          Reference · the five a day, one step at a time · {view} · {theme.name}
        </p>
        <KidFiveADay
