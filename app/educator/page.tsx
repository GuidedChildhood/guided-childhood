...
function ErrorBox({ message }: { message?: string }) {
  ...
  return (
    <div style={{
      ...
      fontSize: 'var(--text-md)', color: 'var(--coral-dark, #8F3F04)', lineHeight: 1.6, overflowWrap: 'anywhere',
    }}>
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'var(--cream)', padding: '48px 20px' }}>
        <div style={{ maxWidth: '520px', margin: '0 auto' }}>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-md)', color: 'var(--ink-soft)', lineHeight: 1.6, marginBottom: '24px' }}>
          ...
          <form action={createSchool} style={{ ...card, display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <label style={{ fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 'var(--text-base)', color: 'var(--ink)' }}>
              School name
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.6fr) minmax(240px, 1fr)', gap: '16px', marginBottom: '16px' }}>
          <div style={{
            ...
            <div style={{ position: 'relative' }}>
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-md)', color: 'rgba(255,255,255,0.82)', lineHeight: 1.6, maxWidth: '440px', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                ...
                <Link href="/educator/curriculum" style={{
                  ...
                  textDecoration: 'none', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)',
                }}>
                  See the curriculum
                </Link>
              </div>
          ...
          <div style={{ ...panel, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-base)', color: 'var(--ink-muted)', textAlign: 'center', lineHeight: 1.5 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px', marginBottom: '16px' }}>
          {stats.map(s => (
            <div key={s.label} style={{ ...panel, padding: '18px 20px' }}>
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-3xl)', color: 'var(--ink)', lineHeight: 1.05 }}>{s.figure}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px', marginBottom: '16px' }}>
          {QUICK.map(q => (
            <Link key={q.href} href={q.href} style={{ ...panel, padding: '18px 20px', textDecoration: 'none', display: 'flex', gap: '12px', alignItems: 'center' }}>
              ...
              <span>
                <span style={{ display: 'block', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink)' }}>{q.title}</span>
                <span style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-base)', color: 'var(--ink-muted)' }}>{q.sub}</span>
              </span>
            </Link>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.15fr) minmax(0, 1fr)', gap: '16px', alignItems: 'start' }}>
          ...
          <div style={panel}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)' }}>Ranked by coverage</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {classCoverage.map((c, i) => (
                <Link key={c.id} href={`/educator/classes/${c.id}`} style={{
                  ...
                  <span style={{
                    ...
                    fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-base)', color: 'var(--ink)',
                  }}>{i + 1}</span>
                  <span style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
                      <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink)' }}>
                        {c.name} <span style={{ fontWeight: 600, color: 'var(--ink-muted)', fontSize: 'var(--text-base)' }}>{c.year_group}</span>
                      </span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--green-dark)' }}>{c.taught}/{liveCount}</span>
                    </span>
              ...
              {classCoverage.length === 0 && (
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-md)', color: 'var(--ink-muted)' }}>No classes yet. Add your first below.</p>
              )}
            </div>

            <details style={{ marginTop: '16px' }}>
              <summary style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--green-dark)', cursor: 'pointer' }}>+ Add a class</summary>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {(lessons ?? []).map(l => (
                <div key={l.id} style={{ padding: '14px 16px', borderRadius: '16px', border: '1px solid var(--border)', background: 'var(--warm)' }}>
                  ...
                  <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink)', marginBottom: '10px', lineHeight: 1.3 }}>{l.title}</div>
                  <span style={{ display: 'flex', gap: '16px' }}>
                    <Link href={`/educator/teach/${l.module_id}`} style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)', color: 'var(--green-dark)', textDecoration: 'none' }}>
                      Teach →
                    </Link>
                    <Link href={`/educator/print/${l.module_id}`} style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)', color: 'var(--gold-dark)', textDecoration: 'none' }}>
                      Print pack →
                    </Link>
                  </span>
                </div>
              ...
              {(lessons ?? []).length === 0 && (
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-base)', color: 'var(--ink-muted)' }}>Run the migrations and the lessons appear here.</p>
              )}
            </div>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-base)', color: 'var(--ink-light)', marginTop: '14px', lineHeight: 1.55 }}>
