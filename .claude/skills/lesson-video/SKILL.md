...
- House cast only, and the house cast is the PLANET FRIENDS: Pebble the youngest and most delighted, Bloop small and kind, Orbit the curious explorer, Nova calm and grown, Cosmo the oldest and forward looking, DiGi the guide with the golden star, DiGi Junior for the youngest. Never render DiGi as a robot or an owl. The DiGi Squad children (Sofia, Zara, Oliver, and the never built Vix and Brock) are RETIRED and must never be generated: they were replaced by the Planet Friends, and the eight lesson clips that still show them are being remade (plans/2026-09-07-planet-friends-lesson-animation-system.md). Which friend hosts which module is decided by `character` in shared/schools-curriculum.ts, never by the prompt.
- Identity comes from the reference sheet, never from the prompt. Pass the friend's art as `image_references` on Seedance and describe only the ACTION. A prompt that contains a physical description of a friend is wrong: that is exactly how the old clips drifted, because each one re described the character from scratch and each render read it differently.
