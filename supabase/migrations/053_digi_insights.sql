...
insert into public.digi_wisdom (topic, age_band, what_works, evidence_count)
select v.topic, v.age_band, v.what_works, v.evidence_count
from (values
  ('Structure over willpower'::text, null::text, 'When screens are a battle, changing the setup does the work that nagging cannot. One clear, calm boundary in place beats ten reminders, because it takes the fight off the child and puts it on the routine.'::text, 4),
...
) as v(topic, age_band, what_works, evidence_count)
