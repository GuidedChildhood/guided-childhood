...
Make three or four different personas so groups can compare feeds at the end. All invented, all low-stakes. **Make one pack the new arrival.** One persona who just turned 16 and opened their first social media account: only eight feed cards instead of twelve, and a history card with just five signals, all from week one. The class discovers that on a fresh account every single signal carries huge weight, which is why the first week decides what a feed becomes, and why arriving with a plan beats arriving blank. **Make one pack the success story.** One persona whose feed genuinely serves their goal: they searched and finished on purpose, followed variety, and their feed became an asset (a persona who trained everything toward animation and is now visibly getting better at it, say). Their history card shows mostly deliberate signals. The audit skill has to work in both directions, and the class needs to see a feed that is working, not only feeds that have drifted.
