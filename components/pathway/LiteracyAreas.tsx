...
function StatusChip({ tone, children }: { tone: ChipTone; children: React.ReactNode }) {
  ...
  return (
    <span style={{
      ...
      {tone === 'green' && <span aria-hidden style={{ fontSize: 'var(--text-sm)', lineHeight: 1 }}>✓</span>}
      {children}
    </span>
  )
}
  ...
  return (
    <div style={{ padding: '0 20px', maxWidth: '720px', margin: '0 auto 20px' }}>
      <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: '20px', padding: '22px 20px 8px', boxShadow: '0 4px 0 rgba(26,26,46,0.05)' }}>
        ...
        <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-lg)', color: 'var(--ink-soft)', lineHeight: 1.6, margin: '0 0 12px', maxWidth: '620px' }}>
        ...
        {(() => {
          ...
          return (
            <div style={{
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-lg)', color: 'var(--ink-soft)', lineHeight: 1.55, margin: '9px 0 0' }}>
                {allGreen
            ...
            return (
              <div key={area.key} style={{ display: 'flex', alignItems: 'flex-start', gap: 14, padding: '18px 0', borderTop: '1px solid var(--border)' }}>
                ...
                <span style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: 'var(--text-md)', color: 'var(--ink-muted)', lineHeight: 1.6, marginTop: 6 }}>
          ...
          const inner = (
            <>
              ...
              <span style={{ flex: 1, minWidth: 0 }}>
                ...
                {live?.value && (
                  <span style={{ display: 'block', fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', color: 'var(--ink)', lineHeight: 1.2, marginTop: 7 }}>
                    {live.value}
                  </span>
                )}
                {bar && <ProgressBar done={bar.done} total={bar.total} tone={chipTone} />}
                <span style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: 'var(--text-lg)', color: 'var(--ink-soft)', lineHeight: 1.6, marginTop: 7 }}>
