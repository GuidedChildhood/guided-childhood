...
export default function MeetTheFriends({ childName }: { childName?: string | null }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: 'var(--lift)',
      borderRadius: 'var(--radius-card)', padding: '18px 18px 16px', marginBottom: '22px',
    }}>
      ...
      <p style={{ fontSize: 'var(--text-lg)', color: 'var(--ink-soft)', lineHeight: 1.55, margin: '0 0 14px' }}>
      ...
      <div style={{ display: 'flex', gap: '14px', overflowX: 'auto', paddingBottom: '4px', scrollbarWidth: 'none' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '7px', flexShrink: 0, width: 62 }}>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--ink-muted)', textAlign: 'center', lineHeight: 1.2 }}>Your guide</span>
        </div>
        ...
        {STAGE_CHARACTERS.map(friend => (
          <div key={friend.key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '7px', flexShrink: 0, width: 62 }}>
            ...
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink)' }}>{friend.name}</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--ink-muted)', textAlign: 'center', lineHeight: 1.2 }}>{friend.ages.replace('Ages ', '')}</span>
          </div>
        ))}
      </div>
    </div>
