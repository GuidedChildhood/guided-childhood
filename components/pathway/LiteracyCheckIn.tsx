...
export default function LiteracyCheckIn({ stageId }: { stageId: number }) {
  ...
  return (
    <div style={{ padding: '0 20px', maxWidth: '720px', margin: '0 auto 20px' }}>
      <div style={{ background: 'var(--terracotta-lt)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-card)', padding: '16px 18px' }}>
        ...
        {result ? (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            <span aria-hidden style={{ flexShrink: 0, width: 34, height: 34, borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: result.grade === 'green' ? 'var(--retro-green, #2F8F6B)' : '#C0533E', color: '#fff', fontSize: 'var(--text-lg)', fontWeight: 900 }}>
              {result.grade === 'green' ? '✓' : '✕'}
            </span>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-md)', color: 'var(--ink)', lineHeight: 1.55, margin: 0 }}>
              {result.note}{' '}
          ...
          <>
            <p style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-lg)', color: 'var(--ink)', lineHeight: 1.4, margin: '0 0 12px' }}>
              {q.question}
            </p>
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
              <textarea
                ...
                rows={2}
                style={{ flex: 1, padding: '11px 14px', borderRadius: 'var(--radius-tile)', border: 'var(--edge)', background: '#fff', fontFamily: 'var(--font-body)', fontSize: 'var(--text-md)', color: 'var(--ink)', resize: 'none', outline: 'none', lineHeight: 1.5 }}
              />
              <button
                onClick={send}
                disabled={busy || !answer.trim()}
                style={{ flexShrink: 0, background: answer.trim() ? 'var(--terracotta)' : '#fff', color: answer.trim() ? 'var(--ink)' : 'var(--ink-muted)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '11px 18px', cursor: answer.trim() ? 'pointer' : 'default', fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-base)', boxShadow: answer.trim() ? 'var(--lift)' : 'none' }}
              >
