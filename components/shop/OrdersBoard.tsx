    ...
    <>
      {waiting.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', color: 'var(--ink)', margin: 0 }}>
            To make
          </h2>
      ...
      {done.length > 0 && (
        <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', color: 'var(--ink-muted)', margin: '30px 0 12px' }}>
          Posted
        </h2>
      )}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'baseline', marginBottom: 12 }}>
        ...
        <span style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', color: 'var(--ink)' }}>
      ...
      <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 14px' }}>
        {row.lines.map((l, i) => (
          <li key={i} style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-lg)', color: 'var(--ink)', lineHeight: 1.35 }}>
      ...
      {row.childName && (
        <p style={{ fontSize: 'var(--text-md)', fontWeight: 700, color: 'var(--ink)', margin: '0 0 12px', lineHeight: 1.45 }}>
          Personalise for {row.childName}. Print their real stamps.
        </p>
      )}

      <div style={{ background: 'var(--cream)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '13px 15px', marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          ...
          {row.address.length > 0 && (
            <button onClick={copyAddress} style={{
              background: '#fff', border: 'var(--edge)', borderRadius: 10,
        ...
        {row.address.length > 0 ? (
          <div style={{ fontSize: 'var(--text-md)', color: 'var(--ink)', lineHeight: 1.5 }}>
            {row.address.map((line, i) => <div key={i}>{line}</div>)}
          </div>
        ) : (
          <div style={{ fontSize: 'var(--text-md)', color: 'var(--terracotta-dark)', fontWeight: 700, lineHeight: 1.5 }}>
            No address on this order. Email them before making it.
          </div>
        )}
        {row.email && (
          <div style={{ fontSize: 'var(--text-base)', color: 'var(--ink-muted)', marginTop: 8 }}>{row.email}</div>
        )}
      </div>
      ...
      <button
        ...
        style={{
          ...
          color: 'var(--ink)',
          border: 'var(--edge)',
          borderRadius: 'var(--radius-btn)', padding: '13px 22px', cursor: busy ? 'wait' : 'pointer',
