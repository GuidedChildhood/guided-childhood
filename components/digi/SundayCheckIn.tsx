      ...
      <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: '20px', padding: '18px 20px', marginBottom: '20px', boxShadow: '0 2px 4px rgba(26,26,46,0.03), 0 10px 28px -14px rgba(26,26,46,0.12)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '13px' }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.11em', textTransform: 'uppercase', color: 'var(--deep-teal)' }}>This week with DiGi</div>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-lg)', color: 'var(--ink)', lineHeight: 1.2 }}>
          ...
          {plan.map((p, i) => (
            <div key={i} style={{ display: 'flex', gap: '11px', background: 'var(--cream)', borderRadius: '13px', padding: '11px 13px' }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, marginTop: '3px' }}>{p.why}</div>
                {p.expert && <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--terracotta-dark)', marginTop: '5px' }}>{p.expert}</div>}
              </div>
            </div>
          ))}
        </div>
  ...
  if (!open) {
    ...
    return (
      <button onClick={() => { setOpen(true); setStepIdx(0) }} style={{
        ...
        <span style={{ flexShrink: 0, fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)', color: '#fff', background: 'var(--terracotta-dark)', borderRadius: '12px', padding: '10px 15px' }}>
          Start
        </span>
      </button>
    )
    ...
    <div style={{ background: '#fff', border: '1.5px solid var(--terracotta)', borderRadius: '22px', padding: '20px 20px 18px', marginBottom: '20px', boxShadow: '0 2px 4px rgba(26,26,46,0.03), 0 16px 40px -14px rgba(26,26,46,0.16)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '11px', marginBottom: '16px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.11em', textTransform: 'uppercase', color: 'var(--terracotta-dark)' }}>Sunday check in</div>
          {/* Progress dots */}
        ...
        <button onClick={() => setOpen(false)} aria-label="Close" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 'var(--text-lg)', color: 'var(--ink-muted)', flexShrink: 0 }}>✕</button>
      </div>

      {stepIdx === 0 && (
        <div>
          <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', color: 'var(--ink)', margin: '0 0 14px', lineHeight: 1.25 }}>First, how are you this week?</h3>
          ...
          {mood != null && <p style={{ textAlign: 'center', fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--terracotta-dark)', margin: '11px 0 0' }}>{MOODS.find(m => m.v === mood)?.label}</p>}
        </div>
      )}

      {stepIdx === 1 && (
        <div>
          <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', color: 'var(--ink)', margin: '0 0 6px', lineHeight: 1.25 }}>What went well?</h3>
          <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-muted)', margin: '0 0 13px' }}>Tap any that fit, or none. Every week has something.</p>
      ...
      {stepIdx === 3 && (
        <div>
          <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', color: 'var(--ink)', margin: '0 0 13px', lineHeight: 1.25 }}>What do you want next week to feel like?</h3>
      ...
      {stepIdx === 4 && (
        <div>
          <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', color: 'var(--ink)', margin: '0 0 4px', lineHeight: 1.25 }}>DiGi’s plan for your week</h3>
          <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-muted)', margin: '0 0 14px' }}>Small, doable, grounded in the evidence. Agree it and it sits on your Home all week.</p>
          {busy && draftPlan.length === 0 ? (
            <p style={{ fontSize: 'var(--text-md)', color: 'var(--terracotta-dark)', fontWeight: 700 }}>Reading your week and shaping a plan…</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '9px' }}>
              {draftPlan.map((p, i) => (
