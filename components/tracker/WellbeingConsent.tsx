...
export default function WellbeingConsent({ onAgreed }: { onAgreed: () => void }) {
  ...
  return (
    <div style={{ maxWidth: 620, margin: '0 auto', padding: '24px 20px 48px' }}>
      ...
      <div style={{ background: '#fff', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-card)', padding: '18px 18px 20px' }}>
        ...
        <label style={{ display: 'flex', gap: 12, alignItems: 'flex-start', cursor: 'pointer', padding: '12px 14px', borderRadius: 'var(--radius-tile)', background: 'var(--cream)', border: `2px solid ${ticked ? 'var(--terracotta)' : 'var(--ink)'}` }}>
          <input
            type="checkbox"
            checked={ticked}
