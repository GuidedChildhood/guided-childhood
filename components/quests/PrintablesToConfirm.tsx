...
export default function PrintablesToConfirm() {
  ...
  return (
    ...
    <div id="printables-to-confirm" style={{ scrollMarginTop: 84, background: 'var(--terracotta-lt)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-card)', padding: '16px 18px', marginTop: '18px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {items.map(it => (
          <div key={it.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: '#fff', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '12px 14px' }}>
            <span aria-hidden style={{ width: 40, height: 40, borderRadius: 'var(--radius-tile)', background: 'var(--terracotta-lt)', border: 'var(--edge)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxSizing: 'border-box', fontSize: 'var(--text-lg)' }}>{it.emoji ?? '🖍️'}</span>
            ...
            <div style={{ display: 'flex', gap: '7px', flexShrink: 0 }}>
              <button
                ...
                disabled={busy === it.id}
                style={{ background: 'var(--terracotta)', color: 'var(--ink)', border: 'none', borderRadius: '11px', padding: '9px 14px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontSize: 'var(--text-base)', fontWeight: 800, boxShadow: '0 3px 0 var(--terracotta-dark)', whiteSpace: 'nowrap' }}
              >
                {busy === it.id ? '...' : 'Confirm ⭐'}
              </button>
              <button
                ...
                disabled={busy === it.id}
                title="Not done yet"
                style={{ background: '#fff', color: 'var(--ink-muted)', border: 'var(--edge)', borderRadius: '11px', padding: '9px 12px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontSize: 'var(--text-base)', fontWeight: 800, whiteSpace: 'nowrap' }}
              >
                Not yet
              </button>
            </div>
