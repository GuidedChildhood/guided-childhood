...
export default function ScreenBalanceInsight({
  ...
  type Tone = 'good' | 'even' | 'pace' | 'empty'
  const tone: Tone = total === 0 ? 'empty' : realMins > screenMins ? 'good' : realMins === screenMins ? 'even' : 'pace'
  const accent = tone === 'good' ? 'var(--retro-green)' : tone === 'pace' ? 'var(--stage-1-bold)' : 'var(--deep-teal)'
  ...
  const line =
    ...
    : `A quest or a real world job would tip it back. For age ${insight.bandLabel}, around ${insight.guideMins} min of screen a day sits comfortably.`
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--terracotta-dark)', marginBottom: '3px' }}>
            Today&apos;s balance
          </div>
      ...
      <div style={{ position: 'relative', height: 22, borderRadius: '100px', overflow: 'hidden', border: '1.5px solid var(--border)', display: 'flex', marginBottom: '10px' }}>
        <span style={{ width: `${screenPct}%`, background: 'var(--terracotta)', transition: 'width 0.6s ease' }} />
