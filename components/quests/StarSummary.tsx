  ...
  return (
    <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: '18px', padding: '16px 18px', marginBottom: '18px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', marginBottom: '4px' }}>
        <span style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-3xl)', color: 'var(--ink)' }}>⭐ {balanceStars}</span>
        <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink-soft)' }}>= {minutes} minutes ready</span>
      </div>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--ink-muted)', marginBottom: '13px' }}>
      ...
      {timerRunning && (
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--ink-soft)' }}>
              {remainingMs != null ? `${fmt(remainingMs)} left on the clock` : 'timer running'}
            </div>
          </div>
          {remainingMs != null && (
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', color: 'var(--deep-teal)', fontVariantNumeric: 'tabular-nums' }}>
              {fmt(remainingMs)}
            </span>
          )}
        </div>
        ...
        <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--tint-sage)', border: '1.5px solid var(--deep-teal)', borderRadius: '13px', padding: '13px 14px', marginBottom: '14px' }}>
          ...
          {onDismissGoalDone && (
            <button
              ...
              style={{ position: 'absolute', top: 8, right: 10, zIndex: 2, background: 'none', border: 'none', cursor: 'pointer', fontSize: 'var(--text-lg)', lineHeight: 1, color: 'var(--ink-muted)', padding: '2px 4px' }}
            >
              ×
            </button>
          )}
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink)', marginBottom: 4, paddingRight: 18 }}>
            ...
            <button
              onClick={onSetGoal}
              style={{ background: 'var(--terracotta)', color: 'var(--ink)', border: 'none', borderRadius: '11px', padding: '9px 15px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)', boxShadow: '0 3px 0 var(--terracotta-dark)' }}
            >
              Set a new goal
            </button>
          </div>
          ...
          {goalReached && (
            confirmDone ? (
              <div style={{ marginTop: 10 }}>
                <p style={{ fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--ink)', margin: '0 0 8px' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    onClick={markDone}
                    disabled={redeeming}
                    style={{ flex: 1, background: 'var(--terracotta)', color: 'var(--ink)', border: 'none', borderRadius: '11px', padding: '10px', cursor: redeeming ? 'default' : 'pointer', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)', boxShadow: '0 3px 0 var(--terracotta-dark)', opacity: redeeming ? 0.7 : 1 }}
                  >
              ...
              <button
                ...
                style={{ width: '100%', marginTop: 10, background: 'var(--terracotta)', color: 'var(--ink)', border: 'none', borderRadius: '11px', padding: '11px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-md)', boxShadow: '0 4px 0 var(--terracotta-dark)' }}
              >
