...
export function FishingView({ game, onDone }: { game: FishingGame; onDone: () => void }) {
  ...
  const pondFrame: React.CSSProperties = {
    ...
    height: 'clamp(292px, 72vw, 322px)', borderRadius: 'var(--radius-card)', overflow: 'hidden',
    background: 'linear-gradient(180deg, #E6F1FB 0%, #D3E6F7 100%)',
    border: '2px solid var(--ink, #1A1A2E)', boxShadow: 'var(--lift)',
  }
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'center', marginBottom: '12px' }}>
        ...
        <div style={{
          flex: 1, maxWidth: '280px', background: '#fff', border: 'var(--edge)',
          borderRadius: 'var(--radius-card)', boxShadow: 'var(--lift)', padding: '10px 14px', textAlign: 'left',
        }}>
        ...
        {phases.map((p, i) => {
          ...
          return (
            <button key={p.id} onClick={() => startRound(i)} style={{
              ...
              border: 'var(--edge)',
              borderRadius: 'var(--radius-pill)', padding: '8px 15px',
              boxShadow: active ? 'var(--lift)' : 'none',
            }}>
              {pillLabel(p)}
            </button>
          )
