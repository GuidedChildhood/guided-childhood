import { HappyPaper, Caption, WriteLine, Moon, Star, INK } from './HappyPaper'
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'center', flexShrink: 0, marginTop: 6 }}>
        <svg viewBox="0 0 700 330" style={{ width: 704, height: 332 }} aria-hidden>
          ...
          <ellipse cx="350" cy="308" rx="120" ry="14" fill="#fff" stroke={INK} strokeWidth="4" />
        </svg>
      </div>
      ...
      <div style={{ flex: '1 1 auto', minHeight: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 10, padding: '0 20px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
          <WriteLine label="In our house, phones go to bed at" width="60%" height={40} size={18} value={bedtime?.start} />
          <WriteLine label="and wake up at" height={40} size={18} value={bedtime?.end} />
        </div>
        <WriteLine label="They sleep here:" height={40} size={18} />
        <WriteLine label="And so do the grown ups' phones, signed:" height={40} size={18} />
      </div>
