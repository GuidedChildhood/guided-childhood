...
export default function ManagementReviewPanel({ row }: { row: ManagementReviewRowOrNull }) {
  if (!row) {
    return (
      <div style={{ background: 'var(--cream)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-btn)', padding: '18px 20px', marginBottom: 20 }}>
  ...
  return (
    <div style={{ background: '#fff', border: 'var(--edge)', borderRadius: 'var(--radius-card)', padding: '20px 22px', marginBottom: 22, boxShadow: 'var(--lift)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10, marginBottom: 18 }}>
        {HEADLINE.map(h => {
          const v = val(h.key)
          return (
            <div key={h.key} style={{ background: 'var(--cream)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '10px 12px' }}>
      ...
      {row.findings.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {row.findings.map((f, i) => {
            ...
            return (
              <div key={i} style={{ border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '13px 15px' }}>
                <span style={{ display: 'inline-block', background: tint.bg, color: tint.fg, borderRadius: 'var(--radius-pill)', padding: '3px 10px', fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 7 }}>
                  {f.area}
                </span>
