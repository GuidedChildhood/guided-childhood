...
export default function InsightsBoard() {
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '28px 20px 64px' }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--terracotta-dark)', marginBottom: 8 }}>
        Founder only
      </div>
      ...
      <section style={{ marginBottom: 26 }}>
        ...
        {pulseError && <p style={{ color: 'var(--ink-muted)', fontSize: 'var(--text-base)' }}>Pulse unavailable: {pulseError}</p>}
        {!pulse && !pulseError && <p style={{ color: 'var(--ink-muted)', fontSize: 'var(--text-base)' }}>Reading the whole app...</p>}
        {pulse && (() => {
          ...
          return (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
                {tiles.map(t => (
                  <div key={t.label} style={{ background: 'var(--white,#fff)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 'var(--radius-tile)', padding: '14px 16px' }}>
              ...
              {stageEntries.length > 0 && (
                <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {stageEntries.map(([band, n]) => (
                    <span key={band} style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--ink-soft)', background: 'var(--cream)', border: 'var(--edge)', borderRadius: 'var(--radius-pill)', padding: '4px 11px' }}>
                      {band}: {n}
                    </span>
                  ))}
                </div>
              ...
              {pulse.families === 0 && (
                <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-muted)', marginTop: 10, lineHeight: 1.5 }}>No families yet. Once real families sign up and use the app, this fills in and the DiGi insight below has conversations to read.</p>
              )}
            </>
          )
        })()}
      ...
      {drafts && drafts.length > 0 && (
        <section style={{ marginBottom: 26 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {drafts.map(d => (
              <div key={d.id} style={{ background: '#fff', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '15px 16px' }}>
                ...
                <p style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-lg)', color: 'var(--ink)', margin: '0 0 4px' }}>{d.title}</p>
                <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, margin: '0 0 8px' }}>{d.situation}</p>
                <p style={{ fontSize: 'var(--text-md)', color: 'var(--ink)', lineHeight: 1.5, margin: '0 0 4px' }}><strong>Say this:</strong> {d.say_this}</p>
                <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, margin: '0 0 4px' }}><strong>Not this:</strong> {d.not_this}</p>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => reviewDraft(d.id, 'approve')} disabled={draftBusy === d.id} style={{ flex: 1, background: 'var(--terracotta)', color: 'var(--ink)', border: 'var(--edge)', borderRadius: 11, padding: '9px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-base)', boxShadow: 'var(--lift)', opacity: draftBusy === d.id ? 0.6 : 1 }}>
                    {draftBusy === d.id ? '…' : 'Add to library'}
                  </button>
                  <button onClick={() => reviewDraft(d.id, 'reject')} disabled={draftBusy === d.id} style={{ background: '#fff', color: 'var(--ink-soft)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 11, padding: '9px 16px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)' }}>
                    Reject
                  </button>
                </div>
              </div>
      ...
      {legal && legal.length > 0 && (
        <section style={{ marginBottom: 26 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {legal.map(l => (
              <div key={l.id} style={{ background: '#fff', border: 'var(--edge)', borderLeft: l.confidence === 'high' ? '4px solid var(--terracotta)' : undefined, borderRadius: 'var(--radius-tile)', padding: '15px 16px' }}>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 6, alignItems: 'center' }}>
                  ...
                  {l.effective_on && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--ink)' }}>from {l.effective_on}</span>}
                </div>
                <p style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-lg)', color: 'var(--ink)', margin: '0 0 4px' }}>{l.headline}</p>
                <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, margin: '0 0 6px' }}>{l.summary}</p>
                {l.impact && <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink)', lineHeight: 1.5, margin: '0 0 8px' }}><strong>Worth checking:</strong> {l.impact}</p>}
                {l.url && (
                  <a href={l.url} target="_blank" rel="noreferrer" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--terracotta-dark)', textDecoration: 'underline' }}>
                    Read the source
                  </a>
                )}
                <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                  <button onClick={() => closeLegal(l.id, 'actioned')} disabled={legalBusy === l.id} style={{ flex: 1, background: 'var(--terracotta)', color: 'var(--ink)', border: 'var(--edge)', borderRadius: 11, padding: '9px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-base)', boxShadow: 'var(--lift)', opacity: legalBusy === l.id ? 0.6 : 1 }}>
                    {legalBusy === l.id ? '…' : 'Done, dealt with'}
                  </button>
                  <button onClick={() => closeLegal(l.id, 'dismissed')} disabled={legalBusy === l.id} style={{ background: '#fff', color: 'var(--ink-soft)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 11, padding: '9px 16px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)' }}>
                    Not for us
                  </button>
                </div>
              </div>
      ...
      {scriptReqs && scriptReqs.length > 0 && (
        <section style={{ marginBottom: 26 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {scriptReqs.map(r => (
              <div key={r.id} style={{ background: '#fff', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '13px 15px' }}>
                ...
                <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <a href={`/dashboard/new-script?problem=${encodeURIComponent(r.problem)}`} style={{ background: 'var(--terracotta)', color: 'var(--ink)', border: 'var(--edge)', borderRadius: 11, padding: '8px 15px', textDecoration: 'none', fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-base)', boxShadow: 'var(--lift)' }}>
                    Write this script →
                  </a>
                  <button onClick={() => markReqHandled(r.id)} disabled={reqBusy === r.id} style={{ background: '#fff', color: 'var(--ink-soft)', border: 'var(--edge)', boxShadow: 'var(--lift)', borderRadius: 11, padding: '8px 15px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-base)', opacity: reqBusy === r.id ? 0.6 : 1 }}>
      ...
      {candidates && candidates.length > 0 && (
        <section style={{ marginBottom: 26 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {candidates.map(c => (
              <div key={c.id} style={{ background: 'var(--white,#fff)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10, marginBottom: 4 }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--ink-muted)', flexShrink: 0 }}>{c.source_type}</span>
                </div>
                <p style={{ fontSize: 'var(--text-md)', color: 'var(--ink)', lineHeight: 1.55, margin: '0 0 6px' }}>{c.finding}</p>
                {c.rationale && <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, margin: '0 0 6px', fontStyle: 'italic' }}>Why: {c.rationale}</p>}
                {(c.topics ?? []).length > 0 && (
                  <div style={{ margin: '4px 0 10px', display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    {(c.topics ?? []).slice(0, 6).map(t => (
                      <span key={t} style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--ink-muted)', background: 'var(--cream)', border: 'var(--edge)', borderRadius: 'var(--radius-pill)', padding: '2px 8px' }}>{t}</span>
                    ))}
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => reviewCandidate(c.id, 'approve')} disabled={reviewing === c.id} style={{ flex: 1, background: 'var(--terracotta)', color: 'var(--ink)', border: 'var(--edge)', borderRadius: 11, padding: '9px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-base)', boxShadow: 'var(--lift)', opacity: reviewing === c.id ? 0.6 : 1 }}>
      ...
      <section style={{ marginBottom: 26 }}>
        ...
        {bankError && <p style={{ color: 'var(--ink-muted)', fontSize: 'var(--text-base)' }}>Bank unavailable: {bankError}</p>}
        {!bank && !bankError && <p style={{ color: 'var(--ink-muted)', fontSize: 'var(--text-base)' }}>Reading the bank...</p>}
        {bank && (
          <>
            <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.6, marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {bank.sources.map(s => (
                <div key={s.source} style={{ background: 'var(--white,#fff)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '12px 14px' }}>
                  ...
                  {s.sample && <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, margin: '5px 0 0' }}>{s.sample}</p>}
                  {s.topics.length > 0 && (
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 24 }}>
        ...
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 'var(--text-base)', color: 'var(--ink-soft)', cursor: 'pointer' }}>
          <input type="checkbox" checked={emailIt} onChange={e => setEmailIt(e.target.checked)} />
          Email it to me
        </label>
        <button onClick={run} disabled={loading} className="btn btn-gold" style={{ padding: '10px 20px', fontSize: 'var(--text-base)', marginLeft: 'auto', cursor: 'pointer', opacity: loading ? 0.7 : 1 }}>
          {loading ? 'Reading conversations...' : 'Run the insight'}
        </button>
      </div>

      {error && <p style={{ color: 'var(--danger)', fontSize: 'var(--text-md)', marginBottom: 16 }}>{error}</p>}

      {data && (
        <>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)', marginBottom: 16 }}>
            {data.count} questions over {data.days} days
          </div>

          {data.count === 0 && (
            <div style={{ background: 'var(--cream)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '16px 18px', marginBottom: 20 }}>
          ...
          {/* Summary */}
          {data.report.summary && (
            <div style={{ background: 'var(--tint-sage)', border: 'var(--edge)', borderRadius: 'var(--radius-btn)', padding: '18px 20px', marginBottom: 22 }}>
            ...
            <section style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {recs.map((r, i) => {
                  ...
                  return (
                    <div key={i} style={{ background: 'var(--white,#fff)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '16px 18px', display: 'flex', gap: 14 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                          <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink)' }}>{r.title}</span>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', background: tint.bg, color: tint.fg, padding: '3px 8px', borderRadius: 'var(--radius-pill)' }}>{r.type.replace('_', ' ')}</span>
                        </div>
                        <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.55, margin: 0 }}>{r.why}</p>
                      </div>
                    </div>
                  )
                })}
            ...
            <section style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 10 }}>
                {(data.report.themes ?? []).map((t, i) => (
                  <div key={i} style={{ background: 'var(--white,#fff)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '15px 16px' }}>
                    ...
                    {t.stages && t.stages.length > 0 && (
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)', letterSpacing: '0.04em', marginBottom: 6 }}>{t.stages.join(' · ')}</div>
                    )}
                    {t.example && <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, margin: 0, fontStyle: 'italic' }}>&ldquo;{t.example}&rdquo;</p>}
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {(data.report.gaps ?? []).map((g, i) => (
                  <div key={i} style={{ background: 'var(--white,#fff)', border: 'var(--edge)', borderLeft: `3px solid ${g.coverage === 'none' ? 'var(--danger)' : 'var(--terracotta)'}`, borderRadius: 'var(--radius-tile)', padding: '14px 16px' }}>
                    ...
                    {g.whatParentsAsk && <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink)', lineHeight: 1.5, margin: '0 0 3px' }}>{g.whatParentsAsk}</p>}
                    {g.note && <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-muted)', lineHeight: 1.5, margin: 0 }}>{g.note}</p>}
                  </div>
                ))}
              </div>
            </section>
      ...
      <section style={{ marginTop: 40, paddingTop: 28, borderTop: 'var(--edge)' }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 18 }}>
          ...
          <button onClick={embedMemories} disabled={qLoading !== null} className="btn btn-outline" style={{ padding: '10px 18px', fontSize: 'var(--text-base)', cursor: 'pointer', opacity: qLoading ? 0.7 : 1 }}>
            {qLoading === 'embed' ? 'Embedding...' : 'Embed memories'}
          </button>
        </div>
        {embedStatus && (
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', color: 'var(--ink-muted)', marginBottom: 16 }}>{embedStatus}</p>
        )}

        {qError && <p style={{ color: 'var(--danger)', fontSize: 'var(--text-md)', marginBottom: 16 }}>{qError}</p>}
          ...
          <div style={{ marginBottom: 22 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {evalRun.results.map(r => {
                ...
                return (
                  <div key={r.id} style={{ background: 'var(--white,#fff)', border: 'var(--edge)', borderRadius: 10 }}>
                    <button
                    ...
                    >
                      <span style={{ fontSize: 'var(--text-md)' }}>{r.safetyPass && r.rubricScore >= 0.75 ? '✓' : !r.safetyPass ? '⚠' : '•'}</span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', color: 'var(--ink)', minWidth: 130 }}>{r.id}</span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, color: r.safetyPass ? 'var(--ink-soft)' : 'var(--danger)' }}>{Math.round(r.score * 100)}%</span>
                      <span style={{ fontSize: 'var(--text-base)', color: 'var(--ink-muted)', lineHeight: 1.4, flex: 1, minWidth: 0 }}>
                    ...
                    {open && (
                      <div style={{ borderTop: '2px dotted rgba(26,26,46,0.18)', padding: '12px 14px 14px' }}>
                        ...
                        <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, margin: '0 0 12px' }}>{r.prompt}</p>
                        <p style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--ink-muted)', margin: '0 0 5px' }}>
                          DiGi said
                        </p>
                        <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink)', lineHeight: 1.6, margin: 0, whiteSpace: 'pre-wrap' }}>
        ...
        {wisdom && (
          <div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)', marginBottom: 10 }}>
              {wisdom.written} patterns written from {wisdom.signals} de-identified signals
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {wisdom.rows.map((w, i) => (
                <div key={i} style={{ background: 'var(--white,#fff)', border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '13px 15px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                    <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', color: 'var(--ink)' }}>{w.topic}</span>
                    {w.age_band && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.06em', color: 'var(--ink-muted)', background: 'var(--tint-sage)', padding: '2px 7px', borderRadius: 'var(--radius-pill)' }}>{w.age_band}</span>}
                  </div>
                  <p style={{ fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.5, margin: 0 }}>{w.what_works}</p>
                </div>
              ))}
            </div>
          </div>
...
function Stat({ label, value, tone }: { label: string; value: string; tone: 'good' | 'warn' | 'bad' }) {
  ...
  return (
    <div style={{ background: bg, border: 'var(--edge)', borderRadius: 'var(--radius-tile)', padding: '12px 16px', minWidth: 110 }}>
