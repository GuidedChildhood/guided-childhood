...
export default function InsightsBoard() {
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '28px 20px 64px' }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--terracotta-dark)', marginBottom: 8 }}>
        Founder only
      </div>
      ...
      <p style={{ fontSize: 17, color: 'var(--ink-soft)', lineHeight: 1.6, marginBottom: 16 }}>
      ...
      <section style={{ marginBottom: 26 }}>
        ...
        {pulseError && <p style={{ color: 'var(--ink-muted)', fontSize: 15 }}>Pulse unavailable: {pulseError}</p>}
        {!pulse && !pulseError && <p style={{ color: 'var(--ink-muted)', fontSize: 15 }}>Reading the whole app...</p>}
        {pulse && (() => {
          ...
          return (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
                {tiles.map(t => (
                  <div key={t.label} style={{ background: 'var(--white,#fff)', border: '1px solid var(--border)', borderRadius: 14, padding: '14px 16px' }}>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--ink-muted)' }}>{t.label}</div>
                    ...
                    {t.sub && <div style={{ fontSize: 14, color: 'var(--ink-soft)', marginTop: 2 }}>{t.sub}</div>}
                  </div>
                ))}
              </div>
              {stageEntries.length > 0 && (
                <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {stageEntries.map(([band, n]) => (
                    <span key={band} style={{ fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 700, color: 'var(--ink-soft)', background: 'var(--cream)', border: '1px solid var(--border)', borderRadius: 100, padding: '4px 11px' }}>
                      {band}: {n}
                    </span>
                  ))}
                </div>
              ...
              {pulse.families === 0 && (
                <p style={{ fontSize: 15, color: 'var(--ink-muted)', marginTop: 10, lineHeight: 1.5 }}>No families yet. Once real families sign up and use the app, this fills in and the DiGi insight below has conversations to read.</p>
              )}
            </>
          )
        })()}
            ...
            {drafts.map(d => (
              <div key={d.id} style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 14, padding: '15px 16px' }}>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--terracotta-dark)' }}>{d.stage_id}{d.category ? ` · ${d.category}` : ''}</span>
                  {d.grounded_in && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 700, color: 'var(--ink-muted)' }}>grounded in {d.grounded_in}</span>}
                </div>
                <p style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 19, color: 'var(--ink)', margin: '0 0 4px' }}>{d.title}</p>
                <p style={{ fontSize: 15.5, color: 'var(--ink-soft)', lineHeight: 1.5, margin: '0 0 8px' }}>{d.situation}</p>
                <p style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.5, margin: '0 0 4px' }}><strong>Say this:</strong> {d.say_this}</p>
                <p style={{ fontSize: 15.5, color: 'var(--ink-soft)', lineHeight: 1.5, margin: '0 0 4px' }}><strong>Not this:</strong> {d.not_this}</p>
                <p style={{ fontSize: 15, color: 'var(--ink-muted)', lineHeight: 1.5, margin: '0 0 4px' }}><strong>Why it works:</strong> {d.why_it_works}</p>
                <p style={{ fontSize: 15, color: 'var(--ink-muted)', lineHeight: 1.5, margin: '0 0 12px' }}><strong>Try tonight:</strong> {d.tonight}</p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => reviewDraft(d.id, 'approve')} disabled={draftBusy === d.id} style={{ flex: 1, background: 'var(--terracotta)', color: 'var(--ink)', border: 'none', borderRadius: 11, padding: '9px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 15, boxShadow: '0 3px 0 var(--terracotta-dark)', opacity: draftBusy === d.id ? 0.6 : 1 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {scriptReqs.map(r => (
              <div key={r.id} style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 14, padding: '13px 15px' }}>
                <p style={{ fontSize: 17, fontWeight: 700, color: 'var(--ink)', lineHeight: 1.45, margin: 0 }}>&ldquo;{r.problem}&rdquo;</p>
                {r.closest && (
                  <p style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--ink-muted)', margin: '6px 0 0' }}>Closest we have: {r.closest}</p>
                )}
                <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <a href={`/dashboard/new-script?problem=${encodeURIComponent(r.problem)}`} style={{ background: 'var(--terracotta)', color: 'var(--ink)', border: 'none', borderRadius: 11, padding: '8px 15px', textDecoration: 'none', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 15, boxShadow: '0 3px 0 var(--terracotta-dark)' }}>
                    Write this script →
                  </a>
                  <button onClick={() => markReqHandled(r.id)} disabled={reqBusy === r.id} style={{ background: '#fff', color: 'var(--ink-soft)', border: '1.5px solid var(--border)', borderRadius: 11, padding: '8px 15px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15, opacity: reqBusy === r.id ? 0.6 : 1 }}>
            ...
            {candidates.map(c => (
              <div key={c.id} style={{ background: 'var(--white,#fff)', border: '1.5px solid var(--terracotta)', borderRadius: 14, padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10, marginBottom: 4 }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12.5, fontWeight: 700, color: 'var(--ink-muted)', flexShrink: 0 }}>{c.source_type}</span>
                </div>
                <p style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.55, margin: '0 0 6px' }}>{c.finding}</p>
                {c.rationale && <p style={{ fontSize: 14, color: 'var(--ink-soft)', lineHeight: 1.5, margin: '0 0 6px', fontStyle: 'italic' }}>Why: {c.rationale}</p>}
                {(c.topics ?? []).length > 0 && (
                  <div style={{ margin: '4px 0 10px', display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    {(c.topics ?? []).slice(0, 6).map(t => (
                      <span key={t} style={{ fontFamily: 'var(--font-mono)', fontSize: 11.5, fontWeight: 700, color: 'var(--ink-muted)', background: 'var(--cream)', border: '1px solid var(--border)', borderRadius: 100, padding: '2px 8px' }}>{t}</span>
                    ))}
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => reviewCandidate(c.id, 'approve')} disabled={reviewing === c.id} style={{ flex: 1, background: 'var(--terracotta)', color: 'var(--ink)', border: 'none', borderRadius: 11, padding: '9px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 15, boxShadow: '0 3px 0 var(--terracotta-dark)', opacity: reviewing === c.id ? 0.6 : 1 }}>
                    {reviewing === c.id ? 'Adding...' : 'OK, add to bank'}
                  </button>
                  <button onClick={() => reviewCandidate(c.id, 'reject')} disabled={reviewing === c.id} style={{ background: '#fff', color: 'var(--ink-soft)', border: '1.5px solid var(--border)', borderRadius: 11, padding: '9px 16px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15 }}>
                    Reject
                  </button>
                </div>
              </div>
      ...
      <section style={{ marginBottom: 26 }}>
        ...
        {bankError && <p style={{ color: 'var(--ink-muted)', fontSize: 15 }}>Bank unavailable: {bankError}</p>}
        {!bank && !bankError && <p style={{ color: 'var(--ink-muted)', fontSize: 15 }}>Reading the bank...</p>}
        {bank && (
          <>
            <p style={{ fontSize: 15.5, color: 'var(--ink-soft)', lineHeight: 1.6, marginBottom: 12 }}>
              ...
              {bank.sources.map(s => (
                <div key={s.source} style={{ background: 'var(--white,#fff)', border: '1px solid var(--border)', borderRadius: 12, padding: '12px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10 }}>
                    ...
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12.5, fontWeight: 700, color: 'var(--ink-muted)', flexShrink: 0 }}>
                      {s.type} · {s.findings}
                    </span>
                  </div>
                  {s.sample && <p style={{ fontSize: 14.5, color: 'var(--ink-soft)', lineHeight: 1.5, margin: '5px 0 0' }}>{s.sample}</p>}
                  {s.topics.length > 0 && (
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 24 }}>
        ...
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 15, color: 'var(--ink-soft)', cursor: 'pointer' }}>
          <input type="checkbox" checked={emailIt} onChange={e => setEmailIt(e.target.checked)} />
          Email it to me
        </label>
        <button onClick={run} disabled={loading} className="btn btn-gold" style={{ padding: '10px 20px', fontSize: 15.5, marginLeft: 'auto', cursor: 'pointer', opacity: loading ? 0.7 : 1 }}>
          {loading ? 'Reading conversations...' : 'Run the insight'}
        </button>
      </div>

      {error && <p style={{ color: 'var(--danger)', fontSize: 16, marginBottom: 16 }}>{error}</p>}

      {data && (
        <>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--ink-muted)', marginBottom: 16 }}>
          ...
          {data.report.summary && (
            <div style={{ background: 'var(--tint-sage)', border: '1px solid var(--border)', borderRadius: 16, padding: '18px 20px', marginBottom: 22 }}>
              <p style={{ fontSize: 17.5, lineHeight: 1.6, color: 'var(--ink)', fontWeight: 500, margin: 0 }}>{data.report.summary}</p>
            </div>
          )}
                ...
                {recs.map((r, i) => {
                  ...
                  return (
                    <div key={i} style={{ background: 'var(--white,#fff)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 18px', display: 'flex', gap: 14, boxShadow: '0 2px 12px rgba(26,26,46,0.05)' }}>
                      <span style={{ width: 26, height: 26, borderRadius: '50%', flexShrink: 0, background: 'var(--ink)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 15 }}>{i + 1}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                          <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 17, color: 'var(--ink)' }}>{r.title}</span>
                        ...
                        <p style={{ fontSize: 15.5, color: 'var(--ink-soft)', lineHeight: 1.55, margin: 0 }}>{r.why}</p>
                      </div>
                    </div>
                  )
                })}
                ...
                {(data.report.themes ?? []).map((t, i) => (
                  <div key={i} style={{ background: 'var(--white,#fff)', border: '1px solid var(--border)', borderRadius: 14, padding: '15px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
                      <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 16.5, color: 'var(--ink)' }}>{t.label}</span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 15, fontWeight: 700, color: 'var(--terracotta-dark)' }}>{t.count}</span>
                    </div>
                    {t.stages && t.stages.length > 0 && (
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11.5, color: 'var(--ink-muted)', letterSpacing: '0.04em', marginBottom: 6 }}>{t.stages.join(' · ')}</div>
                    )}
                    {t.example && <p style={{ fontSize: 14.5, color: 'var(--ink-soft)', lineHeight: 1.5, margin: 0, fontStyle: 'italic' }}>&ldquo;{t.example}&rdquo;</p>}
                  </div>
                ))}
              </div>
            </section>
                ...
                {(data.report.gaps ?? []).map((g, i) => (
                  <div key={i} style={{ background: 'var(--white,#fff)', border: '1px solid var(--border)', borderLeft: `3px solid ${g.coverage === 'none' ? 'var(--danger)' : 'var(--terracotta)'}`, borderRadius: 12, padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                      <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 16, color: 'var(--ink)' }}>{g.topic}</span>
                    ...
                    {g.whatParentsAsk && <p style={{ fontSize: 15, color: 'var(--ink)', lineHeight: 1.5, margin: '0 0 3px' }}>{g.whatParentsAsk}</p>}
                    {g.note && <p style={{ fontSize: 14.5, color: 'var(--ink-muted)', lineHeight: 1.5, margin: 0 }}>{g.note}</p>}
                  </div>
                ))}
              </div>
            </section>
      ...
      <section style={{ marginTop: 40, paddingTop: 28, borderTop: '1px solid var(--border)' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--ink-soft)', lineHeight: 1.6, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 18 }}>
          <button onClick={runEvals} disabled={qLoading !== null} className="btn btn-gold" style={{ padding: '10px 18px', fontSize: 15, cursor: 'pointer', opacity: qLoading ? 0.7 : 1 }}>
            {qLoading === 'evals' ? 'Running the evals...' : 'Run safety evals'}
          </button>
          <button onClick={rebuildWisdom} disabled={qLoading !== null} className="btn btn-outline" style={{ padding: '10px 18px', fontSize: 15, cursor: 'pointer', opacity: qLoading ? 0.7 : 1 }}>
            {qLoading === 'wisdom' ? 'Rebuilding wisdom...' : 'Rebuild shared wisdom'}
          </button>
          <button onClick={embedMemories} disabled={qLoading !== null} className="btn btn-outline" style={{ padding: '10px 18px', fontSize: 15, cursor: 'pointer', opacity: qLoading ? 0.7 : 1 }}>
            {qLoading === 'embed' ? 'Embedding...' : 'Embed memories'}
          </button>
        </div>
        {embedStatus && (
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: 13.5, color: 'var(--ink-muted)', marginBottom: 16 }}>{embedStatus}</p>
        )}

        {qError && <p style={{ color: 'var(--danger)', fontSize: 16, marginBottom: 16 }}>{qError}</p>}
        ...
        {wisdom && (
          <div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--ink-muted)', marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {wisdom.rows.map((w, i) => (
                <div key={i} style={{ background: 'var(--white,#fff)', border: '1px solid var(--border)', borderRadius: 12, padding: '13px 15px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                    <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 16, color: 'var(--ink)' }}>{w.topic}</span>
                  ...
                  <p style={{ fontSize: 15, color: 'var(--ink-soft)', lineHeight: 1.5, margin: 0 }}>{w.what_works}</p>
                </div>
              ))}
            </div>
          </div>
...
function Stat({ label, value, tone }: { label: string; value: string; tone: 'good' | 'warn' | 'bad' }) {
  ...
  return (
    <div style={{ background: bg, borderRadius: 12, padding: '12px 16px', minWidth: 110 }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11.5, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: fg, marginBottom: 3 }}>{label}</div>
      <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 22, color: 'var(--ink)' }}>{value}</div>
    </div>
  )
}
