    ...
    <div style={{ background: '#fff', border: '1.5px solid var(--terracotta)', borderRadius: '20px', padding: '18px 20px', marginBottom: '20px', boxShadow: '0 2px 4px rgba(26,26,46,0.03), 0 12px 30px -14px rgba(26,26,46,0.16)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--terracotta-dark)' }}>New from DiGi</div>
      ...
      <p style={{ fontSize: '16px', color: 'var(--ink-soft)', lineHeight: 1.55, margin: '0 0 14px' }}>{show.body}</p>
      <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
        <Link href={show.href} onClick={seen} style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', background: 'var(--terracotta)', color: 'var(--ink)', textDecoration: 'none', borderRadius: '14px', padding: '12px 18px', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '16px', boxShadow: '0 4px 0 var(--terracotta-dark)' }}>
          Have a look <span aria-hidden>→</span>
        </Link>
        <button onClick={seen} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '13.5px', fontWeight: 700, color: 'var(--ink-muted)' }}>Got it</button>
      </div>
    </div>
  )
}
