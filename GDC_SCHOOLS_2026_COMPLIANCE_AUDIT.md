...
- **The judged lesson quality findings in section 6 were not re run.** The counted checks were, and the first run of them was wrong. The lesson council (`scripts/council.mjs`, the arithmetic checks only) was run over all 29 modules on 20 September against a fixture, and the fixture's copies of the 25 older modules predated migrations 309 to 316, so it reported prose at 9.81 and, after migration 318 trimmed five new KS4 concept slides to 103 words or fewer, 9.95. The same rule run against the production table the same afternoon gives **prose 9.52: 357 of 375 prose slides within the ceiling, 18 over.** Six of the eighteen are EYFS and KS1 slides that were over the 12 word decoding ceiling before this audit began. **Twelve are KS2 to KS4 concept slides that the clauses appended by migrations 309, 310, 311 and 316 pushed over the 105 word projector ceiling, to between 106 and 207 words** (ks4-17 s7 and s13, ks4-16 s7, s16 and s20, ks3-12 s10, ks3-11 s19, ks4-18 s11, ks2-07 s11, ks3-14 s15, ks2-08 s18, ks2-06 s7). The ceiling was measured on a 1920 by 1080 wall, where a slide past it clips its text and can push the Continue control off the screen, so those twelve are a classroom defect and not a score. They are open, named in `plans/decisions.md`, and the honest fix is to carry the appended clause in the teacher's script, which the attestation reads and the teacher says word for word, rather than on the wall. Migration 318's five slides are within the ceiling, the 147 phrase attestation was re run after it (147 checked, 0 not found), and the four new modules pass the same module contract as the other twenty five. That is the extent of the claim.
