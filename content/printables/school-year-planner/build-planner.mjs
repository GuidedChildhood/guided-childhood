...
await page.pdf({ path: DIR + '/product/school-year-planner-A4.pdf', format: 'A4', printBackground: true });
await page.pdf({ path: DIR + '/product/school-year-planner-USLetter.pdf', format: 'Letter', printBackground: true });
await page.evaluate(() => document.body.classList.add('ink'));
await page.pdf({ path: DIR + '/product/school-year-planner-A4-ink-friendly.pdf', format: 'A4', printBackground: true });
...
for (const [id, name] of [['s1','homework-week'], ['s2','homework-first'], ['s3','reading-log']]) {
