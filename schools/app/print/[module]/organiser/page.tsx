...
export default async function KnowledgeOrganiserPage({ params }: { params: Promise<{ module: string }> }) {
  ...
  return (
    <main style={{ maxWidth: '740px', margin: '0 auto', background: '#fff', color: 'var(--ink)', padding: '24px 8px 60px' }}>
      ...
      <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'var(--text-xl)', margin: '4px 0 2px' }}>{lesson.title}</h1>
      ...
      <div style={{ ...box, border: '2px solid var(--gold, #F2C94C)', background: 'var(--cream)' }}>
        ...
        <p style={{ ...body, fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'var(--text-md)', marginTop: '6px' }}>
      ...
      <div style={{ ...box, border: '2px solid var(--green-dark, #2E7D5A)' }}>
        ...
        {tool.strapline && <p style={{ ...body, fontSize: 'var(--text-sm)', color: 'var(--ink-muted)', marginTop: '6px' }}>{tool.strapline}</p>}
      </div>
      ...
      <p style={{ ...mono, fontSize: 'var(--text-sm)', textAlign: 'center', marginTop: '10px' }}>
