...
import { worksheetItems } from '@/lib/worksheet'
import { friendFor, printRegister, mono, display, text, FriendArt, FriendStrip, PrintSheet, Box, WriteLines, BigBox, TickRow, Number, Stamp, ColourStar } from '@/components/print/kit'
...
// ring the child's passport sticker goes in. The register decides the
// sizes: a Reception booklet has one idea a page, boxes to draw in and a
// star to colour; a Year 10 booklet is quieter and denser, and the same
// friend is a small mark. Generated from the lesson row; nothing typed twice.
...
export default async function PupilBookletPage({ params }: { params: Promise<{ module: string }> }) {
  ...
  // The passport page this lesson fills, and the area on it, so the last
  // page can hold the ring the sticker from the passport sheet goes in.
  ...
  return (
    <main style={{ maxWidth: '740px', margin: '0 auto', background: '#fff' }}>
        ...
        {quote && (
          <div className="gc-avoid-break" style={{ display: 'flex', gap: '16px', alignItems: 'center', background: friend.soft, border: `2px solid ${friend.accent}`, borderRadius: `${reg.radius}px`, padding: '16px 20px', marginTop: '18px' }}>
            ...
            <div>
              ...
              <p style={{ ...text, fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: young ? 'var(--text-xl)' : 'var(--text-lg)', lineHeight: 1.3 }}>{quote.text}</p>
            </div>
          </div>
        )}
        {young && <BigBox label="Draw the thing we talked about" height={reg.boxHeight} friend={friend} />}
      </PrintSheet>

      {/* Follow along: my verdicts. */}
      <PrintSheet footer={footer}>
        <FriendStrip friend={friend} register={reg} eyebrow="During the lesson · my answers" />
        ...
        <p style={{ ...text, fontSize: reg.body, color: 'var(--ink-soft)', marginBottom: '14px' }}>
          {young ? 'Tick your answer for each card. Then say why.' : 'Tick your verdict for each card, then write your reason. A verdict without a reason does not count.'}
        </p>
        {items.map(it => (
          <Box key={it.n} radius={reg.radius}>
            ...
            {verdicts.length > 0 && <TickRow options={verdicts} friend={friend} big={young} />}
      ...
      {/* The mission page, and the passport ring. */}
        ...
        <div className="gc-avoid-break" style={{ display: 'flex', flexWrap: 'wrap', gap: '18px 22px', alignItems: 'center', justifyContent: 'center', marginTop: '26px', padding: '18px 20px', border: `2px solid ${friend.accent}`, borderRadius: `${reg.radius + 4}px`, background: friend.soft }}>
          ...
          <div style={{ flex: '1 1 200px' }}>
            ...
            <p style={{ ...text, fontSize: reg.body, color: 'var(--ink-soft)', marginTop: '4px' }}>
              {friend.name} is proud of you.{page && area ? ` This lesson fills the ${page.page} page of your passport: ${AREAS[area].name}. Your sticker is on the passport sheet.` : ''}
            </p>
          </div>
          {page && area && (
            <Stamp friend={friend} label={`${AREAS[area].short}`} size={young ? 40 : 32} hint={young ? 'Sticker here' : 'Your sticker'} />
          )}
        </div>
