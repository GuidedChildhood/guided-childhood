    ...
    <main style={{ minHeight: '100vh', background: 'var(--cream)', padding: PAGE_SHELL }}>
      <div style={{ maxWidth: '1080px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '44px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, ...PAGE.hero, color: 'var(--ink)', marginBottom: 'var(--space-3)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-3)', marginBottom: '18px' }}>
          {PRICING_BANDS.map(band => (
            <div key={band.key} style={{
              ...
              {/* "Most schools" used to be absolutely positioned in the corner, and
                  at the grid's 200px minimum column it sat straight on top of the
                  PRIMARY label (found by the 1440 render on 18 September 2026, not
                  by reading the code, because at a wide viewport the two clear each
                  other). In flow it cannot collide at any width, which is worth
                  more than the corner. */}
              <span style={{ ...eyebrow, color: 'var(--terracotta)', fontSize: 'var(--text-xs)', alignSelf: 'flex-end', visibility: band.featured ? 'visible' : 'hidden' }} aria-hidden={!band.featured}>Most schools</span>
        ...
        <div style={{ background: 'var(--terracotta-lt)', border: '1px solid var(--border)', borderRadius: 'var(--radius-card)', padding: '26px 24px', marginBottom: '48px' }}>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-md)', color: 'var(--ink-soft)', lineHeight: 1.7, marginBottom: '18px', maxWidth: '640px' }}>
            ...
            lead actually loses their term. Four things, and you can read three of them right now
          ...
          <ol style={{ margin: 0, paddingLeft: '20px', display: 'grid', gap: 'var(--space-3)' }}>
            {[
              ...
              { h: 'The page your designated safeguarding lead can file', b: 'Every safeguarding flagged module, the statutory ground it stands on, and the note its teachers get before they teach it. Two standing rules on every one: content is age appropriate and never graphic, and the platform records no disclosures, so every concern follows your own reporting systems. Written to be referenced from your safeguarding policy.', href: '/hub/dsl', cta: 'Open the safeguarding crosswalk' },
            ].map(item => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 'var(--space-4)', alignItems: 'start' }}>
          <div>
            ...
            <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', marginBottom: '20px' }}>
              {LICENCE_INCLUDES.map(item => (
                <li key={item} style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'flex-start', fontFamily: 'var(--font-body)', fontSize: 'var(--text-base)', color: 'var(--ink-soft)', lineHeight: 1.55 }}>
