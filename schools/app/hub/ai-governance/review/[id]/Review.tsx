    ...
    return (
      <main style={{ minHeight: '100vh', background: 'var(--cream)', padding: '48px 20px' }}>
        <div style={{ maxWidth: '560px', margin: '0 auto', ...panel, textAlign: 'center' }}>
          <strong style={{ ...labelStyle, fontSize: 'var(--text-md)' }}>That review is not on this device.</strong>
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--cream)', padding: '28px 20px 80px' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <h1 style={{
          fontFamily: 'var(--font-display)', fontWeight: 900, ...PAGE.section,
          ...
          {steps.map(s => {
            ...
            return (
              <button
                ...
                style={{
                  ...btnQuiet, whiteSpace: 'nowrap', fontSize: 'var(--text-sm)', padding: '8px 14px',
        ...
        {step === 'about' && (
          <div style={{ ...panel, display: 'grid', gap: 'var(--space-4)' }}>
            ...
            <div>
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-sm)', color: 'var(--ink-soft)', margin: '8px 0 0', lineHeight: 1.55 }}>
                ...
                {KEY_STAGES.map(ks => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...btnQuiet, fontSize: 'var(--text-sm)', padding: '8px 14px',
                        background: on ? 'var(--ink)' : '#fff', color: on ? '#fff' : 'var(--ink)',
                        borderColor: on ? 'var(--ink)' : 'var(--border)',
                      }}
                    >{ks}</button>
        ...
        <div className="gc-gov-steps" style={{ display: 'flex', gap: 'var(--space-3)', marginTop: '20px', flexWrap: 'wrap' }}>
        ...
        {questions.map(q => {
          ...
          return (
            <div key={q.id} style={panel}>
              <div style={{ ...labelStyle, fontSize: 'var(--text-md)', lineHeight: 1.45 }}>{q.prompt}</div>

              {q.guidance && (
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-sm)', color: 'var(--ink-soft)', lineHeight: 1.55, margin: '6px 0 0' }}>
                  {q.guidance}
                </p>
              )}

              {q.kind === 'yesno' ? (
                <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap', marginTop: '12px' }}>
                  {ANSWERS.map(opt => {
              ...
              {q.evidence && (
                <label style={{ display: 'block', marginTop: '10px' }}>
                  ...
                  <input
                    ...
                    style={{ ...input, marginTop: '5px', fontSize: 'var(--text-base)' }}
                  />
                </label>
              )}
            </div>
